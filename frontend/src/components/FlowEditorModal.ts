...
export const openFlowEditor = (flow: any) => {
        ...
        form.innerHTML = `
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-2">
                        <label class="block text-xs font-semibold text-gray-500 uppercase">Flow Name</label>
                ...
                <div class="space-y-3 bg-indigo-50/30 p-4 rounded-2xl border border-indigo-100/50">
                    <div class="flex justify-between items-center">
                        <label class="block text-xs font-bold text-indigo-600 uppercase tracking-widest">Triggers (Keywords)</label>
                        <button id="addTriggerBtn" class="text-[10px] bg-white border border-indigo-200 text-indigo-600 px-2 py-1 rounded-lg hover:bg-indigo-50 transition-colors">+ New Trigger</button>
                    </div>
                    <div id="triggersList" class="space-y-2">
                        ${triggers.map((t, idx) => `
                            <div class="flex items-center gap-2 bg-white p-2 rounded-xl border border-indigo-100 shadow-sm">
                                <input data-idx="${idx}" type="text" value="${t.keyword}" class="trigger-keyword flex-1 bg-transparent border-none text-xs outline-none font-medium" placeholder="Keyword (e.g. hola)">
                                <select data-idx="${idx}" class="trigger-match text-[10px] bg-indigo-50 text-indigo-700 px-2 py-1 rounded outline-none border-none font-bold">
                                    <option value="CONTAINS" ${t.matchType === 'CONTAINS' ? 'selected' : ''}>Contains</option>
                                    <option value="EXACT" ${t.matchType === 'EXACT' ? 'selected' : ''}>Exact</option>
                                </select>
                        ...
                        `).join('')}
                        ${triggers.length === 0 ? '<p class="text-[10px] text-gray-400 text-center py-2 italic font-medium">No triggers configured. This flow won\'t start automatically.</p>' : ''}
                    </div>
                </div>
                ...
                <div class="space-y-3">
                    <div class="flex justify-between items-center">
                        <label class="block text-xs font-semibold text-gray-500 uppercase">Steps Sequence</label>
                        <button id="addStepBtn" class="text-[10px] bg-indigo-50 text-indigo-600 px-2 py-1 rounded hover:bg-indigo-100">+ Add Step</button>
                    </div>
                    ...
                    <div id="stepsList" class="space-y-3 pb-4">
                        ${steps.map((step, idx) => `
                            <div class="group bg-white border border-gray-100 rounded-xl p-3 shadow-sm hover:border-indigo-200 transition-all relative">
                                <div class="flex justify-between items-start mb-2">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <select data-idx="${idx}" class="step-type text-[10px] bg-transparent font-semibold uppercase tracking-wider text-indigo-600 outline-none cursor-pointer">
                                            ...
                                            <option value="PTT" ${step.type === 'PTT' ? 'selected' : ''}>Voice Note (PTT)</option>
                                        </select>
                                    </div>
                                ...
                                <textarea data-idx="${idx}" placeholder="Content..." class="step-content w-full bg-gray-50/50 border-none rounded-lg p-2 text-sm outline-none resize-none focus:bg-gray-50">${step.content || ''}</textarea>
                                
                                ${['IMAGE', 'AUDIO', 'PTT'].includes(step.type) ? `
                                    <input data-idx="${idx}" type="text" placeholder="Media URL..." value="${step.mediaUrl || ''}" class="step-url w-full mt-2 bg-gray-50/50 border-none rounded-lg px-2 py-1 text-[10px] font-mono outline-none">
                                ` : ''}

                                <div class="mt-3 flex items-center gap-4 border-t border-gray-50 pt-2">
                                    <div class="flex items-center gap-2">
                                        <span class="text-[9px] text-gray-400 uppercase">Wait</span>
                                        ...
                                        <span class="text-[9px] text-gray-400">ms</span>
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <span class="text-[9px] text-gray-400 uppercase">Jitter</span>
                ...
                <div class="flex justify-between pt-4 border-t border-gray-100">
                    <button id="deleteFlowBtn" class="${isNew ? 'hidden' : ''} text-red-500 hover:text-red-700 text-xs font-medium px-2 py-1">Delete Flow</button>
    ...
    const bindEvents = () => {
        ...
        (form.querySelector("#saveBtn") as HTMLElement).onclick = async () => {
            ...
            try {
                ...
                btn.disabled = false;
                btn.textContent = isNew ? "Create Flow" : "Save Changes";
            }
        };

        if (!isNew) {
            (form.querySelector("#deleteFlowBtn") as HTMLElement).onclick = async () => {
                if (confirm("Delete this flow and its steps?")) {
    ...
    const modal = new Modal(isNew ? "Create New Flow" : "Edit Flow", form);
    render();
};
