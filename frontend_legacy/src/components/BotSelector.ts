...
import { openBotSettings } from "./BotSettingsModal";
...
export const BotSelector = () => {
    ...
    const render = () => {
        ...
        // Settings Button (if bot selected)
        if (current) {
            const settingsBtn = document.createElement("button");
            settingsBtn.className = "w-full text-left px-3 py-2 rounded-lg text-sm text-gray-600 hover:bg-gray-50 flex items-center gap-2";
